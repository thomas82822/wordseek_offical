...
composer.command("daily", async (ctx) => {
  ...
  try {
    ...
    return ctx.reply(
      `${pe("🎯")} WordSeek of the Day started! Guess the 5-letter word. You have 6 attempts. Good luck!`,
    );
  } catch (error) {
...
composer.command("pausedaily", async (ctx) => {
  ...
  try {
    ...
    return ctx.reply(
      `${pe("✅")} Your WordSeek of the Day game has been paused. You can now play regular WordSeek.\n\nTo play today's WordSeek again, use /daily (your previous attempts will still count).`,
    );
  } catch (error) {
...
CommandsHelper.addNewCommand("pausedaily", "Pause WordSeek of the Day game", true);
