...
composer.command("seekauth", async (ctx) => {
  ...
  if (!isAdmin && !isSystemAdmin) {
    return await ctx.reply(
      `${pe("❌")} You don't have permission to use this command. Only administrators can manage authorized users.`,
      replyConfig,
    );
  }
  ...
  if (action === "list") {
    ...
    if (authorizedUsers.length === 0) {
      return await ctx.reply(
        `${pe("📋")} No authorized users in this chat.`,
        replyConfig,
      );
    }
    ...
    return await ctx.reply(
      `<b>${pe("🔐")} Authorized Users for Seek Game</b>\n\n${userList}`,
      replyConfig,
    );
  }
  ...
  if (action === "remove") {
    ...
    if (!targetUser) {
      return await ctx.reply(`${pe("❌")} User not found.`, replyConfig);
    }
    ...
    if (deleted.numDeletedRows === 0n) {
      return await ctx.reply(`${pe("❌")} This user is not authorized.`, replyConfig);
    }
    ...
    return await ctx.reply(
      `${pe("✅")} <b>${userName}</b> is no longer authorized to end the game.`,
      replyConfig,
    );
  }
  ...
  if (!targetUser) {
    return await ctx.reply(
      `${pe("❌")} Could not identify the user. Please mention with @username, provide user ID, or reply to their message.`,
      replyConfig,
    );
  }
  ...
  if (existing) {
    return await ctx.reply(
      `${pe("⚠️")} <b>${targetUser.name}</b> is already authorized to end the game in this chat.`,
      replyConfig,
    );
  }
  ...
  return await ctx.reply(
    `${pe("✅")} <b>${userName}</b> is now authorized to end the game without voting!`,
    replyConfig,
  );
});

CommandsHelper.addNewCommand("seekauth", "Manage users authorized to end the seek game (admin only)", true);
