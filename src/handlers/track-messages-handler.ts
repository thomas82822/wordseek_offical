...
const checkGreetingCombo = async (ctx: Context, text: string) => {
  ...
  const targets: number[] = env.ANTICHEAT_LOGS_CHANNEL
    ? [env.ANTICHEAT_LOGS_CHANNEL]
    : env.ADMIN_USERS;
...
composer.use(async (ctx, next) => {
  // ── SPEED FIX: respond first, track second ───────────────────────────────
  // next() is called immediately so the bot replies with zero extra latency.
  // All tracking / anti-cheat logic runs in a fire-and-forget background task
  // so it never blocks gameplay or command responses.
  await next();

  const chatId = ctx.chat?.id;
  if (!chatId) return;

  // Background work — does NOT delay the user's response
  (async () => {
    ...
    // Chat tracking — Redis lookup only if tracking is active for this chat
