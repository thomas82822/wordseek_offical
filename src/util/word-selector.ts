...
import everydaySixWords from "../data/everyday-six.json";
import everydayFiveWords from "../data/everyday-five.json";
import everydayFourWords from "../data/everyday-four.json";

export type WordLength = 4 | 5 | 6;

// ── Win word strategy ────────────────────────────────────────────────────────
// 75% of the time, pick from the curated "everyday" list — simple words that
// everyone knows (think, brave, party, world, etc.).
// 25% of the time, fall back to the broader common list for variety.
// This prevents obscure words from being selected as win words.

const EVERYDAY_WORD_LIST: Record<WordLength, string[]> = {
  4: everydayFourWords,
  5: everydayFiveWords,
  6: everydaySixWords,
};
...
export class WordSelector {
  ...
  async getRandomWord(
    ...
    // 75% of the time use everyday words (simple, familiar words everyone knows).
    // 25% of the time use the broader common list for variety.
    const useEveryday = randomInt(0, 100) < 75;
    const wordList = useEveryday
      ? EVERYDAY_WORD_LIST[wordLength]
      : WORD_LIST[wordLength];
    ...
    try {
      ...
      const availableWords = wordList.filter(
        (word) => !usedSet.has(word.toLowerCase()),
      );
